...
function PickAndUploadFile(editor, forImage) {
			...
			editor._.imagepasteFormInitFn = fnInitPicker = CKEDITOR.tools.addFunction( function() {
				window.setTimeout(function() {
					...
					picker.onchange=function() {
						...
						filePickerEditor.fire('startDirectUpload', { name: IEUpload_fileName } );
						this.form.submit();
					};
					picker.click();
				}, 100);
	...
	if (!filePicker)
	{
		...
		filePicker.addEventListener("change", function () {
				...
				// Upload the file
				uploadFile( filePickerEditor, file, id, fileName, filePickerForceLink );
			}
		);
		if (window.opera)
		{
	...
	function setUrl( fileUrl, data )
	{
		...
		filePickerEditor.fire('endDirectUpload', { name: IEUpload_fileName, ok: (!!fileUrl) } );
...
//		PickAndUploadFile(editor, false);
//	}
//};

var addImageCmd = {
	exec: function( editor ) {
		PickAndUploadFile(editor, true);
	}
};
		...
		editor.on( 'paste', function(e) {
			...
			html = html.replace( /<img src="data:image\/.{3,4};base64,.*?" alt="">/g, function( img )
				...
				*/
					...
					var xhr = createXHRupload(editor, id, fileName);
...
//		});
		...
		CKEDITOR.on( 'droppedFile', function() {
...
function createXHRupload(editor, id, fileName, forceLink)
{
	...
	// Upload the file
	xhr.open("POST", getUploadUrl(editor, 2, forImage) );
	xhr.onload = function() {
		...
		editor.fire('endDirectUpload', { name: fileName, ok: (!!fileUrl) } );
		if (xhr.status!=200)
		{
...
function uploadFile(editor, file, id, fileName, forceLink)
{
	var xhr = createXHRupload(editor, id, fileName, forceLink);
