...
class FormRender extends React.Component {
    ...
    async getCacheData() {
        return await this.helper.request(
            "v1",
            this.appUrl + "/cache/" + this.state.cacheId, {},
            "get"
        );
    }
    ...
    async getWorkflow() {
        ...
        return await this.helper.request(
            "v1",
            this.appUrl + "/form/" + this.state.formId + "/workflow", {},
            "get"
        );
    }
    async getForm() {
    ...
    async getFileDataById() {
        ...
        return await this.helper.request(
            "v1",
            this.appUrl + "/file/" + this.props.fileId + "/data", {},
            "get"
        );
    }
    async getStartFormWorkflow() {
        ...
        return await this.helper.request(
            "v1",
            this.appUrl + "/workflow/" + this.state.workflowId + "/startform", {},
            "get"
        );
    }
            ...
            return await this.helper
                ...
                .then(async (response) => {
                    if (response.status == "success") {
                        ...
                        var cache = await this.deleteCacheData()
                            .then((response2) => {
                                that.showFormLoader(false, 0);
                                if (response2.status == "success") {
                                    this.stepDownPage();
        ...
        if (this.state.parentWorkflowInstanceId && !this.state.isDraft) {
            ...
            this.getStartFormWorkflow()
                .then((response) => {
                    ...
                    parsedData.workflow_uuid ?
                        (parsedData.workflow_uuid = parsedData.workflow_uuid) :
                        null;
                    this.setState({
                        content: template,
                            ...
                            .then((cacheResponse) => {
                                if (Object.keys(cacheResponse.data).length > 1) {
                                    ...
                                    if (cacheResponse.data) {
                                        form
                                            .setSubmission({
                                                data: this.formatFormData(cacheResponse.data),
                                            })
                                            .then((respone) => {
                                                that.processProperties(form);
            ...
            this.getActivityInstance()
                .then((response) => {
                    if (response.status == "success") {
                        ...
                        that.setState({
                            data: that.formatFormData(JSON.parse(response.data.data)),
                        });
            ...
            this.getInstanceData().then((response) => {
                if (response.status == "success" && response.data.workflow_id) {
                    ...
                    that.setState({ activityId: response.data.activity_id });
                    that.setState({
                        data: this.addAddlData(JSON.parse(response.data.data)),
                    });
                            ...
                            .then(function (response) {
                                if (response.status == "success") {
                                    ...
                                    if(response.message == "Errors"){
                                        if(response.data){
                                            var errorMsg = (response.data.errors) ? response.data.errors : null;
                ...
                if (response.status == "success") {
                    ...
                    this.createForm().then((form) => {
                        if (Object.keys(parsedData).length > 1) {
                            ...
                            if (parsedData.data) {
                                form
                                    .setSubmission({ data: this.formatFormData(parsedData.data) })
                                    .then((respone) => {
                                        that.processProperties(form);
                                    });
                            } else {
