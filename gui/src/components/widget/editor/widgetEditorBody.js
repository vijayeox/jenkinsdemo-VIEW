...
class WidgetEditorBody extends AbstractEditor {
  constructor(props) {
    super(props);
    this.type =
      props.type === "inline" || props.type === "html" ? "widget" : props.type;
  ...
  getTargetFieldList() {
    ...
    if (targetColumn) {
      targetColumn.map((item, index) => {
        targetFieldsCopy.push({
          label: item.dataFields.categoryX,
          value: item.dataFields.valueY,
        });
      });
  ...
  configurationChanged = (evt) => {
    ...
    this.setState((state) => {
      ...
      state.errors.configuration =
        "" === value ? thiz.ERRORS.CHART_CONFIGURATION_NEEDED : null;
      return state;
    });
  ...
  refreshWidgetPreview = () => {
    ...
    if (!errorMessage) {
      ...
      previewElement = document.querySelector(
        "div#" + this.state.widgetType + "Preview"
      );
      ...
      try {
        ...
        if (this.state.widgetType === "chart")
          this.amChart = WidgetRenderer.renderAmCharts(
            previewElement,
            config,
            props,
            this.data
          ...
          WidgetRenderer.renderAggregateValue(
            previewElement,
            config,
            props,
            this.data
  ...
  refreshViews = () => {
    ...
    let hasDrillDown =
      ...
      configuration["oxzion-meta"]["drillDown"]
        ? true
        : false;
    if (hasDrillDown) {
  ...
  areAllFieldsValid = () => {
    ...
    let isConfigurationTabValid = this.isConfigurationTabValid(
      this.state,
      false
    );
    ...
    switch (this.state.selectedTab) {
      ...
      case "table":
        if (!isConfigurationTabValid) {
          ...
          switchToTab = findInvalidTab([
            { query: isQueryTabValid },
            { expression: isExpressionTabValid },
          ]);
        }
        break;
      case "widget":
        if (!isConfigurationTabValid) {
          ...
          switchToTab = findInvalidTab([
            { widget: isConfigurationTabValid },
            { expression: isExpressionTabValid },
          ]);
        }
        break;
      ...
      case "query":
        if (!isQueryTabValid) {
          ...
          switchToTab = findInvalidTab([
            { expression: isExpressionTabValid },
            { [this.state.widgetType]: isConfigurationTabValid },
          ]);
        }
        break;
      case "expression":
  ...
  async getTargetData(widgetUuid) {
    ...
    window
      .postDataRequest(
        `analytics/target/getwidgettarget/widgetId/` + widgetUuid
      )
  ...
  refreshPreview() {
    if (
      ...
      this.state.selectedTab === "inline" ||
      this.state.selectedTab === "html"
    ) {
      //refresh preview
      this.refreshWidgetPreview();
  ...
  applyTarget = () => {
    ...
    if (
      Object.keys(errors.target[SINGLELEVEL]).length > 0 ||
      Object.keys(errors.target[SINGLELEVEL]).length > 0
    ) {
    ...
    let target = "";
    let targetFieldName = this.state.targetFields[0]
      ? this.state.targetFields[0].label
      : "aggregate";
    let targetVal;
    if (!this.state.singleTarget) {
      ...
      for (var i in multiLimit) {
        ...
        if (params["target_id"]) {
          ...
          window
            .postDataRequest(
              "analytics/target/" + params["uuid"],
              params,
              "put"
            )
            .then(function (response) {
              console.log(response);
              window.postDataRequest(
                "analytics/target/createwidgettarget",
                targetParams,
                "put"
              );
            });
          ...
          target = window
            .postDataRequest("analytics/target", params, "post")
            .then(function (response) {
              console.log(response);
      ...
      if (this.state.singleLimit.target_id) {
        window
          .postDataRequest(
            "analytics/target/" + this.state.singleLimit.target_uuid,
            params,
            "put"
          )
          .then((response) => {
  ...
  handleTargetInputChange = (e) => {
    ...
    if (!this.state.singleTarget) {
      if (this.state.multiLimit[e.target.name]) {
        ...
        multiLimit[e.target.name][e.target.id] = e.target.value;
        multiLimit[e.target.name]["group_name"] =
          this.state.targetFields[0].label;
    ...
    if (e) {
      ...
      if (value != "") {
        ...
        if (limit == SINGLELEVEL) {
          !regex_condition.test(value)
            ? (errors["target"][SINGLELEVEL][name] =
                "*Please enter decimal values")
            : delete errors["target"][SINGLELEVEL][name];
        } else {
          !regex_condition.test(value)
            ? (errors["target"][MULTILEVEL][`${name}_${id}`] =
                "*Please enter decimal values")
            : delete errors["target"][MULTILEVEL][`${name}_${id}`];
        }
      } else {
        if (limit == SINGLELEVEL) {
      ...
      this.state.singleLimit.red_limit == "" &&
        (errors["target"][SINGLELEVEL]["red_limit"] = "*Field cannot be empty");
      this.state.singleLimit.yellow_limit == "" &&
        (errors["target"][SINGLELEVEL]["yellow_limit"] =
          "*Field cannot be empty");
      this.state.singleLimit.green_limit == "" &&
        (errors["target"][SINGLELEVEL]["green_limit"] =
          "*Field cannot be empty");

      //checking if all fields are entered in multiple target

      Array.isArray(this.props.widget.data) &&
        this.props.widget.data.map((item, index) => {
          let value =
            this.state.multiLimit[
              this.state.targetFields[0].label + "_" + index
            ];
          if (
            value == undefined ||
            (value != undefined && value.red_limit == "")
          ) {
            errors.target[MULTILEVEL][
              `${this.state.targetFields[0].label}_${index}_red_limit`
            ] = "*Field cannot be empty";
          }
          if (
            value == undefined ||
            (value != undefined && value.green_limit == "")
          ) {
            errors.target[MULTILEVEL][
              `${this.state.targetFields[0].label}_${index}_green_limit`
            ] = "*Field cannot be empty";
          }
          if (
            value == undefined ||
            (value != undefined && value.yellow_limit == "")
          ) {
            errors.target[MULTILEVEL][
              `${this.state.targetFields[0].label}_${index}_yellow_limit`
            ] = "*Field cannot be empty";
          }
        });
  ...
  getTemplateListOptions = (templateList) => {
    ...
    templateList.map((temp) =>
      templateOptions.push({ value: temp, label: temp.split(".")[0] })
    );
  ...
  render() {
    ...
    function getQuerySelectOptoins(keyPrefix) {
      ...
      let options = [
        <option
          value=""
          key={keyPrefix + "00000000-0000-0000-0000-000000000000"}
        >
          -Select query-
        </option>,
      ];
          ...
          <div className="form-group row" key={"qs-00-" + i}>
            <div className="col-7" key={"qs-01-" + i}>
              <Select
                ...
                ref={"query" + i}
                onChange={(e) => {
                  thiz.querySelectionChanged(e, i);
                }}
                isDisabled={thiz.state.readOnly}
                value={
                  thiz.state.queries[i]
                    ? getQuerySelectOptoins("qs-03-" + i).filter(
                        (option) =>
                          option.value == thiz.state.queries[i]["uuid"]
                      )
                    : ""
                }
              ...
              <Overlay
                id={"query-overlay" + i}
                target={thiz.refs["query" + i]}
                show={thiz.state.errors.queries[i] != null}
                placement="right"
              >
                {(props) => (
                  <Tooltip
            ...
            <div className="dash-manager-buttons">
              ...
              <button
                type="button"
                className="btn btn-primary widget-action-btn"
                title="Apply filter to query"
                onClick={() => thiz.applyFilterToQuery(i)}
                key={"qs-05-" + i}
                disabled={
                  thiz.state.queries[i]
                    ? thiz.state.queries[i].uuid
                      ? thiz.state.readOnly
                      : true
                    : true
                }
              ...
              <button
                type="button"
                className="btn btn-primary widget-action-btn"
                title="Apply grouping to query"
                onClick={() => thiz.applyGroupingToQuery(i)}
                key={"qs-08-" + i}
                disabled={
                  thiz.state.queries[i]
                    ? thiz.state.queries[i].uuid
              ...
              <button
                type="button"
                className="btn btn-primary widget-action-btn"
                title="Sort query result"
                onClick={() => thiz.applySortingToQuery(i)}
                key={"qs-11-" + i}
                disabled={
                  thiz.state.queries[i]
                    ? thiz.state.queries[i].uuid
              ...
              {/* </div> */}
              {
                thiz.state.queries.length > 1 && (
                  ...
                  <button
                    ...
                    className="btn btn-primary widget-action-btn"
                    title="Delete query"
                    onClick={() => thiz.deleteQuery(i)}
                    key={"qs-14-" + i}
                    disabled={thiz.state.readOnly}
                  >
                    <span
            ...
            <div className="card-body">
              <div className="form-group row" style={{ marginBottom: "0px" }}>
                <Tabs
                  ...
                  onSelect={this.configurationTabSelected}
                >
                      <Tab
                        eventKey={this.state.widgetType}
                        title={capitalizeFirstLetter(this.state.widgetType)}
                      >
                  ...
                  <Tab eventKey="query" title="Query">
                    ...
                    {getQuerySelections()}
                    <div
                      className="col-1 dash-manager-buttons"
                      style={{ paddingLeft: "0px", float: "left" }}
                    >
                      <button
                        type="button"
                        className="btn btn-primary widget-action-btn add-query-button"
                        title="Add query"
                    ...
                    >
                      <div className="col-12 no-left-padding no-right-padding">
                        <textarea
                          ...
                          onBlur={this.expressionBlurred}
                          disabled={this.state.readOnly}
                        />
                        <Overlay
                          id="expression-overlay"
                          target={this.refs.expression}
                          show={this.state.errors.expression != null}
                          placement="top"
                        >
                          {(props) => (
                            <Tooltip
                  ...
                  <Tab eventKey="drilldown" title="Drill Down">
                    <div className="drilldown-div">
                      ...
                      <Form.Group as={Row}>
                        ...
                        <Col md="3" lg="3">
                          <Select
                            ...
                            onChange={(e) =>
                              this.handleSelect(e, "drillDownWidgetType")
                            }
                            value={
                        ...
                        <Col md="3" lg="6">
                          <Select
                            placeholder={
                              this.state.drillDownWidgetType.value ==
                              "dashboard"
                                ? "Choose Dashboard"
                                : "Choose Widget"
                            }
                            ...
                            isDisabled={this.state.readOnly}
                            onChange={(e) =>
                              this.handleSelect(e, "drillDownWidget")
                            }
                            value={
                              this.state.drillDownWidgetType.value ==
                              "dashboard"
                                ? this.props.selectableDashboardOptions.filter(
                                    (option) =>
                                      option.value == this.state.drillDownWidget
                                  )
                                : this.props.selectableWidgetOptions.filter(
                                    (option) =>
                                      option.value == this.state.drillDownWidget
                                  )
                            }
                            options={
                              this.state.filteredWidgetList.length > 0
                                ? this.state.filteredWidgetList
                                : this.props.selectableWidgetOptions
                            }
                          />
                          <Form.Text className="errorMsg">
                        ...
                        <Form.Group as={Row}>
                          ...
                          <Col lg="9">
                            <Form.Control
                              ...
                              onChange={(e) =>
                                this.handleDrillDownInputChange(e)
                              }
                      ...
                      <Button
                        ...
                        disabled={this.state.readOnly}
                        onClick={() => {
                          this.ApplyDrillDown(this.state.widgetType);
                        }}
                      >
                        Apply DrillDown
                      </Button>
                  ...
                  {this.props.widget &&
                    ...
                    (this.props.widget.configuration.series ||
                      this.props.widget.type == "inline") && (
                          ...
                          <div className="col-12">
                            <Form.Group as={Row}>
                              ...
                              <Col lg="9">
                                <Form.Control
                                  ...
                                  disabled={
                                    this.state.readOnly ||
                                    this.state.disabledTargetForm
                                  }
                                  onChange={(e) =>
                                    this.handleTargetSelectChange(e)
                                  }
                                ...
                                <Form.Group as={Row}>
                                  ...
                                  <Col lg="4">
                                    <Form.Control
                                      ...
                                      onChange={(e) =>
                                        this.handleTargetInputChange(e)
                                      }
                                    />
                                    <Form.Text className="errorMsg">
                                      {
                                        this.state.errors.target[SINGLELEVEL][
                                          "red_limit"
                                        ]
                                      }
                                    </Form.Text>
                                  ...
                                  <Col lg="4">
                                    <Form.Control
                                      ...
                                      value={
                                        this.state.singleLimit.red_workflow_id
                                      }
                                      onChange={(e) =>
                                        this.handleTargetInputChange(e)
                                      }
                                ...
                                <Form.Group as={Row}>
                                  ...
                                  <Col lg="4">
                                    <Form.Control
                                      ...
                                      value={
                                        this.state.singleLimit.green_workflow_id
                                      }
                                      onChange={(e) =>
                                        this.handleTargetInputChange(e)
                                      }
                            ...
                            <Button
                              ...
                              disabled={
                                ...
                                Object.keys(
                                  this.state.errors["target"][MULTILEVEL]
                                ).length != 0
                              }
                              onClick={(e) => {
                                this.applyTarget();
                            ...
                            <Overlay
                              id="target_sla-overlay"
                              target={this.refs.target_sla}
                              show={this.state.errors.target_sla != null}
                              placement="top"
                            >
                              {(props) => (
                                <Tooltip
                                  id="target_sla-tooltip"
                                  {...props}
                                  className="error-tooltip"
                                >
                        ...
                        >
                          <div className="col-7">
                            <Select
                              ...
                              onChange={(e) => {
                                this.templateSelectionChanged(e);
                              }}
                              value={selectedTemplate ? selectedTemplate : ""}
                              options={this.getTemplateListOptions(
                                templateList
                              )}
                            />
                          </div>
          ...
          <div className="card" id="previewBox">
            ...
            <div className="card-body">
              ...
              {(this.state.selectedTab === "query" ||
                this.state.selectedTab === "expression") && (
                <div id="queryPreview">
                  <textarea
                    id="queryPreviewText"
                    name="queryPreviewText"
                    className="form-control form-control-sm"
                    style={{ fontFamily: "Monospace" }}
                    value=""
                    disabled={true}
