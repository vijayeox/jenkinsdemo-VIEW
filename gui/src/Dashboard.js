...
class Dashboard extends Component {
                        ...
                        this.setState(
                            {
                                htmlData: response.data.dashboard.content
                                    ? response.data.dashboard.content
                                    : null,
                            },
                            () => {
    ...
    updateGraphWithFilterChanges() {
        let filterParams = extractFilterValues(
            this.props.dashboardFilter,
            this.props.dashboardStack
        );
        let preparedFilter;
        ...
        if (filterParams) {
            if (filterParams.length == 0) {
                ...
                if (this.props.dashboardStack.length > 1) {
                    ...
                    for (
                        let combinedindex = combinedFilter.length - 1;
                        combinedindex >= 0;
                        combinedindex--
                    ) {
                ...
                this.getWidgetByUuid(widgetUUId, filterParams).then((response) => {
                    if (response.status == "success") {
                        response.data.widget &&
                            console.timeEnd(
                                "analytics/widget/" + response.data.widget.uuid + "?data=true"
                            );
                        that.setState({
